...
namespace Tames
{
    public class TameCamera
    {
        ...
        public static Vector3 eyeHeight = 1.5f * Vector3.up;
        ...
        private static bool CheckDistanceAndAngle(GameObject owner, Vector3 position, Vector3 forward, float activeDistance, float activeAngle, Markers.InputSetting.Axis axis)
        {
            ...
            if (activeAngle <= 1)
            {
                if (activeDistance <= 0) return true;
                else return u.magnitude <= activeDistance;
            }
            else
            {
                Vector3 v = axis switch
                {
                    Markers.InputSetting.Axis.X => -owner.transform.right,
                    Markers.InputSetting.Axis.Y => -owner.transform.up,
                    Markers.InputSetting.Axis.Z => -owner.transform.forward,
                    Markers.InputSetting.Axis.NegX => owner.transform.right,
                    Markers.InputSetting.Axis.NegY => owner.transform.up,
                    Markers.InputSetting.Axis.NegZ => owner.transform.forward,
                    _ => owner.transform.position - position
                };
                bool angled = Markers.InputSetting.Axis.X == Markers.InputSetting.Axis.None ||  activeAngle <= Vector3.Angle(v, forward);
                if (activeDistance <= 0) return angled;
                else return u.magnitude <= activeDistance;
            } 
        }
