...
namespace Tames
{
    public class TameLinkManager
    {
        ...
        {
            if (element.qMarker != null)
                if (element.qMarker.cycler != null)
                    PopulateCycle(tgos, tes, element.qMarker.cycler);
            if (element.markerQueue != null)
                PopulateQueue(tgos, tes, element.markerQueue);
        }
        private void PopulateCycle(List<TameGameObject> tgos, List<TameElement> tes, QCycle mc)
        {
            ...
            if (mc != null)
                if (mc.active)
                {
                    ...
                    Debug.Log("mc " + (mc.childrenOf == null) + " " + mc.childrenOf.Length);
