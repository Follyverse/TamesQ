...
namespace Multi
{
    ...
    {
        public ulong[] pressed = new ulong[] { 0, 0, 0 };
        public ulong[] held = new ulong[] { 0, 0, 0 };
        public byte aux = 0;
        public byte grip = 0;
    }
    public class PersonClient
    {
        ...
        public void AddInput(Message m, int sub, bool hold)
        {
            ...
            if (!hold)
                Debug.Log(id + " press ");
            m.AddByte((byte)sub);
            m.AddULong(hold ? input.held[sub] : input.pressed[sub]);
        }
        ...
        public static void FakeReceive(Message m)
        {
            ...
            for (int i = 0; i < changeCount; i++)
            {
                ...
                switch (type)
                {
                    ...
                    case Person.PRESS:
                    case Person.HOLD: m.GetULong(); break;
                }
            }
        }
            ...
            for (int i = 0; i < changeCount; i++)
            {
                ...
                switch (type)
                {
                    ...
                    case Person.ROT:
                        ...
                        break;
                    case Person.PRESS:
                        receivedInput.pressed[sub] |= m.GetULong();
                        inputChanged[sub] = true;
                        Debug.Log("press");
                        break;
                    case Person.HOLD:
                        inputChanged[3 + sub] = true;
                        receivedInput.held[sub] = m.GetULong();
                        break;
                    case Person.AUX:
                        inputChanged[6] = true;
