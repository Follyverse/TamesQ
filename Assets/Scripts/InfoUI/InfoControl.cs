...
namespace InfoUI
{
    public class InfoReference
    {
        ...
        public bool Identify()
        {
          ...
          //      Debug.Log("id: hold " + element.name);
            ...
            QMarker mao = gameObject.GetComponent<QMarker>();
            if (mao != null)
                if(mao.qtype == QType.AlterMaterial)
            {
                foreach (TameAlternative ta in TameManager.altering)
                    if (ta.qMarker.gameObject == gameObject)
                    {
    ...
    public class InfoControl
    {
        ...
        public InfoControl(QMarker m)
        {
            // multiControl = m.multiControl;
              marker = m;
            detached = m.detached;
            if (CoreTame.VRMode && m.position.position.position != InfoPosition.OnObject && m.position.position.position != InfoPosition.WithObject)
            {
            ...
            control = m.pageControl;
            control.AssignControl(InputSetting.ControlType.DualPress);
            areas = new();
            List<QFrame> infos = new List<QFrame>();
            maxLines = 0;
            maxIndex = -1;
            QFrame f, lastFrame = null;
            List<InfoItem> its = new List<InfoItem>();
            for (int i = 0; i < marker.frames.Length; i++)
            {
                f = null;
                if (marker.frames[i].replace == InfoOrder.ReplaceImage && lastFrame != null)
                {
                    if (lastFrame.item.image != null)
                    {
                        f = new QFrame() { marker = marker, index = infos.Count, parent = this, item = marker.frames[i], justImage = true, parentFrame = lastFrame };
                    }
                }
                else if (marker.frames[i].replace != InfoOrder.ReplaceImage)
                {
                    f = new QFrame() { marker = marker, material = material, index = infos.Count, parent = this, item = marker.frames[i] };
                    lastFrame = f;
                }
                if (f != null)
                {
                    infos.Add(f);
                    if (!f.justImage)
                        if (marker.frames[i].lineCount > maxLines) { maxLines = marker.frames[i].lineCount; maxIndex = i; }
                }
            }
            items = its.ToArray();
        ...
        {
            QControl[] mcs = marker.controls;
            foreach (QControl control in mcs)
                if (control.effect == ControlTarget.Visibility)
                {
        ...
        public void Update()
        {
            ...
            //    Debug.Log("curent " + current);
                if (marker.position.position.position == InfoPosition.OnObject)
                {
        ...
        {
            foreach (QFrame frame in frames)
            {
                frame.UpdateColors();
            }
        }
