...
namespace InfoUI
{
   ...
   /*     public static List<Sprite> sprites = new List<Sprite>();
        ...
        public void Initialize(float height)
        {
            if (height < 0)
            {
                if (marker.position.position == InfoPosition.WithObject)
                {
                ...
                else
                {
                    ...
                    margin = Screen.height * marker.position.margin;
                    MoveRects();
        ...
        void ReadText(string text)
        {
            ...
            Color currentColor = marker.colorSetting.foreground;
                ...
                for (int i = 0; i < ta.Length; i++)
                {
                    ...
                    else
                    {
                        ...
                        if (sf == AllStyle)
                        {
                            ...
                            if (hi == -1) lastColor = marker.colorSetting.foreground;
                            else if (hi == 1) lastColor = marker.colorSetting.highlight;
                            else if (lc >= 0) lastColor = marker.colorSetting.others.Length > lc ? marker.colorSetting.others[lc] : marker.colorSetting.highlight;
                            continue;
                        }
                        else
                        {
                        ...
                        if (sf > 0)
                        {
                            ...
                            if (hi == -1) currentColor = marker.colorSetting.foreground;
                            else if (hi == 1) currentColor = marker.colorSetting.highlight;
                            else if (lc >= 0) currentColor = marker.colorSetting.others.Length > lc ? marker.colorSetting.others[lc] : marker.colorSetting.highlight;
                        }
                        ...
                        for (int j = 0; j < brack.Count; j++)
                            if (brack[j].StartsWith("{"))
                            {
                                ...
                                if (prop != InfoReference.RefProperty.None)
                                    words.Add(new WordRef() { style = currentStyle, color = currentColor, text = "", depth = id, prop = prop, spaceBefore = j == 0 });
                                else
        ...
        void MoveRects()
        {
            ...
            switch (marker.position.position)
            {
        ...
        {
            Vector2 s = new Vector2(Screen.width, Screen.height);
            Vector2 m = new Vector2(marker.position.width * Screen.width, h);
            //    m = Vector2.Scale(m, s);
            Vector2 d = s - m;
            //    Debug.Log(marker.position);
            switch (marker.position.position)
        ...
        {
            Rect[] r = ItemRect();
            if (item.image == null || !marker.position.onlyForText)
                return r;

            float nh = r[0].height, nw= r[0].width, nx=r[0].xMin, ny=r[0].yMin;
            switch (marker.position.imagePosition)
            {
                case Markers.ImagePosition.Top:
                case Markers.ImagePosition.Bottom:
                    ny = marker.position.imagePosition == Markers.ImagePosition.Top ? outer.yMin : r[0].yMin;
                    nh = r[0].height + margin;
                    nw = nh / r[0].height * r[0].width;
                    nx = r[0].xMin - (nw - r[0].width) / 2;
                    break;
                case Markers.ImagePosition.TopLeft:
                case Markers.ImagePosition.TopRight:
                    ny = outer.yMin;
                    nx = marker.position.imagePosition == Markers.ImagePosition.TopLeft ? outer.xMin : r[0].xMin;
                    nh = r[0].height + margin;
                    nw = r[0].width + margin;
                    break;
                case Markers.ImagePosition.Left:
                case Markers.ImagePosition.Right:
                    nx = marker.position.imagePosition == Markers.ImagePosition.Right ? r[0].xMin : outer.xMin;
                    nw = r[0].width + margin;
                    nh = nw / r[0].width * r[0].height;
                    ny = r[0].yMin - (nh - r[0].height) / 2;
                    Debug.Log(" infx "+marker.name);
                    break;
                case Markers.ImagePosition.BottomLeft:
                case Markers.ImagePosition.BottomRight:
                    ny = r[0].yMin;
                    nx = marker.position.imagePosition == Markers.ImagePosition.BottomLeft ? outer.xMin : r[0].xMin;
                    nh = r[0].height + margin;
                    nw = r[0].width + margin;
                    break;
            }
        ...
        void PanelizeBorderWithObject()
        {
            ...
            if (marker.colorSetting.image != null)
            {
                ...
                m = (int)(margin / main.height * marker.colorSetting.image.height);
                iRect[4] = new(0, 0, m, m);
        ...
        void PanelizeArea()
        {
            ...
            for (int i = 1; i < context.Length; i++)
            {
                ...
                {
                    wa = (int)(context[i].width / inner.width * (marker.colorSetting.image.width - 2 * m));
                    ha = (int)(context[i].height / inner.height * (marker.colorSetting.image.height - 2 * m));
                    cx = m + (int)((context[i].xMin - inner.xMin) / inner.width * w);
                    cy = m + (int)((context[i].yMin - inner.yMin) / inner.height * h);
                    image.sprite = Sprite.Create((Texture2D)marker.colorSetting.image, new(cx, cy, wa, ha), Vector2.zero);
                    sprites.Add(image.sprite);
                }
            }
        }
        void Panelize()
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    int m = (int)(margin / main.height * marker.colorSetting.image.height);
        ...
        void CreateCanvas()
        {
            ...
            if (marker.position.position != InfoPosition.WithObject) owner.layer = 5; // ui
        ...
        void AdjustOnScreen()
        {
            ...
            if (marker.position.position == InfoPosition.OnObject)
            {
        ...
        float AddLine(int y, int start, int indent, string indentString, out bool created, float width, out bool manualBreak, out int next)
        {
            ...
            if (r > start)
            {
                ...
                if (indentString != "")
                {
                    ...
                    text[i].color = (words[i].style & Highlight) == 0 ? marker.colorSetting.foreground : marker.colorSetting.highlight;
                    text[i].fontStyle = GetStyle(words[i].style);
            ...
            if (marker.colorSetting.image == null)
                for (int i = 0; i < childTypes.Count; i++)
                    if (childTypes[i] == -1)
                    {
                        ...
                        text[childTypes[i]].color = words[childTypes[i]].IsHighlight ? marker.colorSetting.highlight : marker.colorSetting.foreground;
        ...
        public void UpdateChoice()
        {
            ...
            foreach (InfoChoice ch in choice)
            {
                ...
                foreach (int g in ch.indexes)
                    if (text[g] != null) text[g].color = ch.selected ? marker.colorSetting.choice : (words[g].IsHighlight ? marker.colorSetting.highlight : marker.colorSetting.foreground);
