...
namespace Records
{
    ...
    public class VRMap
    {
        ...
        public static bool Gripped(ulong hold, int index)
        {
            if (index == 0)
                return (hold & GL) > 0;
            else
        ...
        public ulong UPressed = 0, UHold = 0;
        private void Act(ulong index, bool[] value)
        {
        ...
        public bool Pressed(ulong index)
        {
            return (UPressed & index) > 0;
        }
        public bool Held(ulong index)
        {
            return (UHold & index) > 0;
        }
        public const ulong Mask = ulong.MaxValue - TL - TR - SLDOWN - SLRIGHT - SLUP - SLLEFT;
        public ulong UPressedMasked
        {
            set
            {
                UPressed = Mask & value;
    ...
    public class GPMap
    {
        ...
        public ulong UPressed, UHold;
        public const ulong Mask = ulong.MaxValue - TL - TR - SLDOWN - SLRIGHT - SLUP - SLLEFT - SRDOWN - SRUP;
        public ulong UPressedMasked
        {
            set
            {
        ...
        private void Act(ulong index, bool pressed, bool hold)
        {
            if ((UPressed & index) > 0) UPressed -= index;
        ...
        private void ActRetro(ulong index, bool pressed)
        {
    ...
    {
        public const int LeftMouse = 61, MiddleMouse = 62, RightMouse = 63;
        public VRMap vrMap;
        public GPMap gpMap;
        ...
        public FrameShot Capture()
        {
            ...
            if (Mouse.current != null)
            {
                hold[61] = Mouse.current.leftButton.isPressed;
                hold[62] = Mouse.current.middleButton.isPressed;
                hold[63] = Mouse.current.rightButton.isPressed;
                pressed[61] = pressed[61] ? false : Mouse.current.leftButton.wasPressedThisFrame;
                pressed[62] = pressed[62] ? false : Mouse.current.middleButton.wasPressedThisFrame;
                pressed[63] = pressed[63] ? false : Mouse.current.rightButton.wasPressedThisFrame;
        ...
        public ulong UPressed
        {
            get
            {
                ...
                for (int i = 0; i < keyCount; i++)
                    r += (pressed[i] ? 1u << i : 0);
                r += pressed[61] ? 1u << 61 : 0;
                r += pressed[62] ? 1u << 62 : 0;
                r += pressed[63] ? 1u << 63 : 0;
                return r;
            }
            set
            {
                ...
                pressed[61] = (value & 1u << 61) > 0;
                pressed[62] = (value & 1u << 62) > 0;
                pressed[63] = (value & 1u << 63) > 0;
            }
        }
        public ulong UHold
        {
            ...
            set
            {
                for (int i = 0; i < keyCount; i++)
                    hold[i] = (value & (1u << i)) != 0;
                hold[61] = (value & 1u << 61) > 0;
                hold[62] = (value & 1u << 62) > 0;
                hold[63] = (value & 1u << 63) > 0;
            }
        }
        public byte Aux
        {
