...
namespace Tames
{
    ...
    public class TameElement : TameThing
    {
        ...
        public List<MaterialReference> scaledMaterials = new List<MaterialReference>();
        ...
        public virtual Updater GetParent(Updater par = null)
        {
            ...
            //    if (name == "distance") Debug.Log("distance = " + (par.isDistance));
        ...
        public virtual void Scale(bool manual)
        {
            if (tameType == TameKeys.Object)
            {
                ...
                if (to.scales)
                {
                    ...
                    for (int i = 0; i < initialTiles.Count; i++)
                        if (scaledMaterials[i].keyExist[TameMaterial.MainTex])
                        {
                            tex = scaledMaterials[i].clone.GetTextureScale(scaledMaterials[i].keys[TameMaterial.MainTex]);
                            if (to.scaleUV == 0) tex.x = s * initialTiles[i]; else tex.y = s * initialTiles[i];
                            scaledMaterials[i].clone.SetTextureScale(scaledMaterials[i].keys[TameMaterial.MainTex], tex);
                            if (manual) Debug.Log("scaling: " + i + " " + name + " " + s);
                        }
                }
            }
        }
        ...
        public static int GetAllParents(Updater[] allEffects, List<TameElement> tes)
        {
            for (int i = 0; i < tes.Count; i++)
            {
                if (tes[i].Manual)
                {
                    if (tes[i].name == "distance") Debug.Log("cus+m: " + tes[i].name);
                ...
                else
                {
                    ...
                    tes[i].CheckStatus();
                    //    if (tes[i].name == "distance") Debug.Log("cus+nm: " + tes[i].name);
        ...
        public void ReadInput(QControl mc)
        {
            if (mc.mode == ControlType.Manual)
        ...
        public void CleanAreas()
        {
                ...
                if (!isSwitch)
                    foreach (TameArea ti in areas)
                        if (ti.geometry == InteractionGeometry.Distance)
                        {
                            ...
                            if (name == "distance") Debug.Log("cus: area dist " + updaters.Count);
                            break;
                        }
            }
            else
            ...
            else if (isDistance)
            {
                for (int i = areas.Count - 1; i >= 0; i--)
                    ...
                    {
                        if (name == "distance") Debug.Log("cus " + i);
                        areas.RemoveAt(i);
                    }
                if (name == "distance") Debug.Log("cus remove");
                //          parents.Clear();
        ...
        public bool PopulateUpdateByMarker(List<TameElement> tes, List<TameGameObject> tgos, QControl mp)
        {
            // TameMaterial tm;
            //   Debug.Log("pop " + name);
            ...
            if (mp.parent != null && mp.mode == ControlType.Element)
            {
        ...
        public void PopulateUpdates(List<TameElement> tes, List<TameGameObject> tgos)
        {
            ...
            QControl[] mcs;
            if (tameType != TameKeys.Material) if(qMarker!=null) SetControls(mcs = qMarker.controls);
            ...
            foreach (QControl mp in updateMarkers)
       ...
       //     if (name == "distance") Debug.Log("cus: pop");
            if (!trackable)
            {
          //      if (name == "distance") Debug.Log("cus: pop track");
                if (tameType == TameKeys.Object)
                {
            //        if (name == "distance") Debug.Log("cus: pop obj");
            ...
            if (updaters.Count == 0)
            {
                if (name == "distance") Debug.Log("cus: count 0");
                if (startingParent.tameType == TameKeys.Time)
                  ...
                  if (name == "distance") Debug.Log("cus: up " + updaters[0].isDistance);
        ...
        public void SetDurations(QProgress mp)
        {
        ...
        public void SetControls(QControl[] mcs)
        {
            for (int i = 0; i < mcs.Length; i++)
            {
                if (mcs[i].effect == ControlTarget.Progress)
                {
                    mcs[i].control.AssignControl(InputSetting.ControlType.DualHold);
                     if (mcs[i].control.mono.Count > 0)
                        updateMarkers.Add(mcs[i]);
                }
                if (mcs[i].effect == ControlTarget.Activation)
                {
                    mcs[i].control.AssignControl(InputSetting.ControlType.Mono);
                    if (mcs[i].control.mono.Count > 0)
                        actMarker = mcs[i];
                }
                if (mcs[i].effect == ControlTarget.Visibility)
                {
                    mcs[i].control.AssignControl(InputSetting.ControlType.Mono);
                    if (mcs[i].control.mono.Count > 0)
                        visMarker = mcs[i];
                }
                if (mcs[i].effect == ControlTarget.Alter)
                {
        ...
        {
            foreach (QControl mc in updateMarkers)
                if (mc != null)
                    ReadInput(mc);
