...
//public bool closed = false;
        ...
        public static bool HandleKey(GameObject g, bool activeMatters)
        {
            if (activeMatters && !g.activeSelf) return false;
            for (int i = 0; i < All.Length; i++)
                if (g.name.ToLower().StartsWith(All[i])) return true;
            return false;
        }
        /// <summary>
