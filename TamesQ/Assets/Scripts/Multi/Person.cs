...
namespace Multi
{
    ...
    public class Person
    {
        ...
        public void SendFrameAsClient(Records.FrameShot prev, Records.FrameShot current)
        {
            ...
            if (NetworkManager.Singleton.Client.IsConnected)
            {
               ...
               //     Debug.Log("local pressed " + id);
                    ...
                    m.AddByte(0);
                    m.AddULong(current.KBPressed);
                }
                if (current.GPPressed != 0)
                {
                    m.AddByte(Person.PRESS);
                    m.AddByte(1);
                    m.AddULong(current.GPPressed);
                }
                if (current.VRPressed != 0)
                {
                    m.AddByte(Person.PRESS);
                    m.AddByte(2);
                    m.AddULong(current.VRPressed);
                }
                for (int i = 0; i < 3; i++)
                    if (holdSendable[i])
                    {
        ...
        {
            ulong v = handIndex == 0 ? Records.VRMap.GL : Records.VRMap.GR;
